...
feature_sets <- c("top6000HVGs", "top8000HVGs", "LowExpressionFilter")
...
hyperparameter_grid <- expand.grid(
  ...
  feature_set = feature_sets # Input feature set names
)

### Call scGeneRanger with hyperparameter grid
load(snakemake@input[[4]])
scGR_results <- sc_RF_RFE_with_tuning_grid(x=data@x_train, y=data@y_train, x_v2=data@x_v2, y_v2=data@y_v2, 
                                           hyper_grid=hyperparameter_grid, input_feature_list=input_features,
                                           folds=fold_indices)
