...
input_files <- list(
  ...
  scGR_bestconfig = snakemake@input[["scGR_bestconfig"]],
  DEGs = snakemake@input[["DEGs"]],
  ElasticNet = snakemake@input[["ElasticNet"]],
...
for (method in names(input_files)) {
  for (file in input_files[[method]]) {
    # Extract cell type from the filepath (assumes cell type is in the file path)
    celltype <- basename(dirname(dirname(file)))
    
    # Skip DEGs method for "CD4T_cell"
    if (method == "DEGs" && grepl("CD4T_cell", file)) {
      next
    }
    
    if (method %in% c("DeepLift", "FeatureAblation")) {
      # Special case: 3 methods per log file
      k_values <- extract_optimal_k(file, num_methods = 3)
      ...
      if (method == "DeepLift") {
        optimal_k_values <- append(optimal_k_values, list(
          ...
          list(Method = "DeepLift (set size from scGR with best config)", Celltype = celltype, k = k_values[3])
        ))
        ...
        optimal_k_values <- append(optimal_k_values, list(
          ...
          list(Method = "FeatureAblation (set size from scGR with best config)", Celltype = celltype, k = k_values[3])
        ))
      }
    } else {
