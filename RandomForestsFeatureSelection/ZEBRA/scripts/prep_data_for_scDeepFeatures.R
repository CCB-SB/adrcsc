...
seu_train <- readRDS(file = snakemake@input[[2]])
...
seu_train <- SetIdent(seu_train, value = "condition")

### Filtering of lowly expressed genes
conditions <- unique(seu_train$condition)
...
#celltype_labels <- plyr::mapvalues(seu_train_filtered$annotation_broad, from = celltype_mapping, to = names(celltype_mapping))
#table(celltype_labels)
#table(seu_train_filtered$annotation_broad)

condition_labels <- seu_train_filtered$condition
...
balanced_labels <- ifelse(balanced_labels == "AD", "AD", "HC")
