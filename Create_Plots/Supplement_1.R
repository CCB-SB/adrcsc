...
supp_data <- read.csv("data/Literature.csv")
this_data = data.frame(
  Paper = "This paper",
  DOI = "",
  method = "sc",
  total.nb.of.samples = 290,
  nb.of.patients = 0,
  nb.of.controls = 0,
  nb.of.cells = 909000,
  Disease = "AD, PD, MCI",
  first.author..short. = "This Dataset",
  year = ""
)
supp_data <- rbind(supp_data, this_data)
supp_data$nb.of.cells[supp_data$nb.of.cells == 0] <- 1
supp_data$Disease <-
  factor(supp_data$Disease,
         levels = c("AD", "PD", "AD, PD", "AD, MCI", "AD, PD, MCI"))
colors <- c("#ff7f00", "#6a3d9a", "#1f78b4", "#a6cee3", "black")
plot <-
  ggplot(supp_data,
         aes(
           x = total.nb.of.samples,
           y = nb.of.cells,
           color = Disease,
           label = paste(first.author..short., year, sep = ", ")
         )) +
  geom_point() +
