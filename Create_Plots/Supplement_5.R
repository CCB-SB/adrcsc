...
degs_collected <-
  degs_collected[degs_collected$contrast == "ADvsHC", ]
degs_collected <- degs_collected[degs_collected$Sex == "female", ]
degs_collected$Celltype_sex <-
  paste(degs_collected$cluster_id, degs_collected$Sex, sep = "~")
...
getSimilarity <- function(data, cell_column) {
  ...
  for (celltype in unique(data[[cell_column]])) {
    print(celltype)
    degs_cellt <-
      data[data[[cell_column]] == celltype, colnames(data) %in% c("logFC", "gene")]
    
    degs_cellt$rank <- NA
    degs_cellt$rank[order((degs_cellt$logFC), decreasing = T)] <-
      1:nrow(degs_cellt)
    
    degs_cellt <-
      degs_cellt[, colnames(degs_cellt) %in% c("rank", "gene")]
    if (is.na(is.na(degs_matrix)[1]) | is.na(degs_matrix)[1]) {
      degs_matrix <- degs_cellt
      colnames(degs_matrix)[colnames(degs_matrix) == "rank"] <-
        celltype
    } else {
  ...
  similarity <-
    similarity[abs(apply(similarity_without_mid, 1, max, na.rm = TRUE)) > 0.5, abs(apply(similarity_without_mid, 1, max, na.rm =
                                                                                           TRUE)) > 0.5]
...
column_ha = HeatmapAnnotation(
  Type = anno_simple(
    str_replace_all(colnames(similarity), "~.*", ""),
    height = unit(1, "cm"),
    col = color_v
  ),
  show_legend = FALSE
...
row_ha = rowAnnotation(
  Type = anno_simple(
    str_replace_all(rownames(similarity), "~.*", ""),
    width = unit(1, "cm"),
    col = color_v
  ),
  show_legend = FALSE
...
degs_collected <- rbind(degs_female, degs_male)
degs_collected <-
  degs_collected[degs_collected$contrast == "PDvsHC", ]
degs_collected <- degs_collected[degs_collected$Sex == "female", ]
degs_collected$Celltype_sex <-
  paste(degs_collected$cluster_id, degs_collected$Sex, sep = "~")
...
degs_collected <- rbind(degs_female, degs_male)
degs_collected <-
  degs_collected[degs_collected$contrast == "MCIvsHC", ]
degs_collected <- degs_collected[degs_collected$Sex == "female", ]
degs_collected$Celltype_sex <-
  paste(degs_collected$cluster_id, degs_collected$Sex, sep = "~")
...
ggsave(
  "figures/Supplement_5_MCI_f.svg",
  as.ggplot(plot5),
  width = 5,
  height = 4
)
...
ggsave(
  "figures/Supplement_5_MCI_m.svg",
  as.ggplot(plot6),
  width = 4,
  height = 4
)
...
svg("figures/Legend_Supplement_5.svg",
    width = 1,
    height = 1.5)
draw(lgd)
dev.off()
