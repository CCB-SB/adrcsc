...
p0 <-
  metadata_new[metadata_new$SCMD %in% patients_with_moreTP &
                 metadata_new$Visit.x == 1, ]
p0 <- p0[!duplicated(p0$SCMD), ]
p1 <-
  metadata_new[metadata_new$SCMD %in% patients_with_moreTP &
                 metadata_new$Visit.x == 2, ]
...
patients_with_1TP <-
  unique(metadata_new$SCMD[metadata_new$SCMD %in% patients_with_1TP &
                             metadata_new$Diagnosis == "Healthy Control"])
p0 <-
  sample(x = patients_with_1TP, size = floor(0.5 * length(patients_with_1TP)))
p0 <- metadata_new[metadata_new$SCMD %in% p0, ]
p0 <- p0[!duplicated(p0$SCMD), ]
p1 <-
  sample(x = patients_with_1TP[!patients_with_1TP %in% p0], size = floor(0.5 *
                                                                           length(patients_with_1TP)))
...
patients_with_1TP <- as.character(count$Var1[count$Freq == 1])
p0 <-
  metadata_new[metadata_new$SCMD %in% patients_with_1TP &
                 metadata_new$Diagnosis == "Healthy Control", ]
p0 <- p0[!duplicated(p0$SCMD), ]
p1 <-
  metadata_new[metadata_new$SCMD %in% patients_with_1TP &
                 metadata_new$Diagnosis == "Parkinson's Disease only", ]
p1 <- p1[!duplicated(p1$SCMD), ]
...
p1 <-
  metadata_new[metadata_new$SCMD %in% patients_with_1TP &
                 metadata_new$Diagnosis == "Alzheimer's disease", ]
p1 <- p1[!duplicated(p1$SCMD), ]
...
plot <-
  ggscatter(
    ...
    add = "reg.line",
    conf.int = TRUE,
    palette = c("black", "darkgray", "red")
  )
ggsave("figures/supplement_10.pdf",
       plot,
       width = 5,
       height = 5)
