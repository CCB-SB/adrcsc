...
pbmc <- readRDS("data/CompleteObjectAnnotated.rds")
...
cell_counts_all <- pbmc@meta.data %>% group_by(L2, SCMD) %>%  summarise(n = n())
...
corr_data <- NA
for (d in c("Mild Cognitive Impairment","Alzheimer's disease","Healthy Control", "Parkinson's Disease only")){
  for (ct in unique(cell_counts_all$L2)){
    for (meta_col in unique(meta_info)){
      if (! is.numeric(meta[[meta_col]][1])) next
      corr <- cor.test(meta[[ct]][meta$Diagnosis == d], meta[[meta_col]][meta$Diagnosis == d])
      m_info <-  meta[[meta_col]][meta$Diagnosis == d]
      data <- data.frame(cor = corr$estimate, p = corr$p.value, Diagnosis = d, celltype = ct , meta = meta_col, l = length( m_info[!is.na(m_info)]))
...
plot <- ggplot(corr_data[corr_data$meta %in% corr_data$meta[corr_data$p.adj < 0.05],], aes(x = celltype, y = meta, fill = cor))+ geom_tile()+ 
...
ggsave("figures/Supplement_11_c.svg", plot, width = 18, height = 5, unit = "cm")

tmp <- corr_data[corr_data$meta %in% corr_data$meta[corr_data$p.adj < 0.05],]
