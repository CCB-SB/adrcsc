...
plot <-
  ggplot(counts[counts$Sex == "male", ], aes(x = cluster_id, y = contrast)) +
  ...
  labs(fill = '#DEGs ') + theme(legend.position = "bottom") + theme(legend.title =
                                                                      element_text(size = 8),
                                                                    legend.text = element_text(size = 6))

plot2 <-
  ggplot(counts[counts$Sex == "female", ], aes(x = cluster_id, y = contrast)) +
  geom_tile(aes(fill = c)) +
  scale_fill_gradient2(
    low = "white",
    high = "#9C489A",
  ...
  scale_x_discrete(name = "", labels = labels_cell[as.character(counts$cluster_id)]) + scale_y_discrete(name = "") + labs(fill =
                                                                                                                            '#DEGs ') +
  theme(
    axis.title.x = element_blank(),
    axis.text.x = element_blank(),
    axis.ticks.x = element_blank()
  ) + theme(legend.position = "bottom") + theme(legend.title = element_text(size =
                                                                              8),
                                                legend.text = element_text(size = 6))
