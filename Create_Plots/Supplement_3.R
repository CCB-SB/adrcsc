...
adrc_obj$celltype_cluster <-
  factor(adrc_obj$celltype_cluster, levels = celltype_cluster_order)
adrc_obj$CellCluster <-
  factor(adrc_obj$L2, levels = CellCluster_order)
adrc_obj$cellt_cl_tmp <-
  factor(adrc_obj$cellt_cl_tmp, levels = labels_cell[celltype_cluster_order])
...
plot1 <-
  DotPlot(
    ...
    feature = c(
      ...
      "GP1BB",
      "GNG11",
      # Megacaryocytes, "AC000093.1", "OAZ1"
      "CD1C",
      "FCER1A",
      "LILRA4",
      "GPR183",
      # Dendritic cells, "GZMB", "ITM2C", "CADM1"
      "NCAM1",
      "NCR1",
      # NK cells #"FCGR3A"
      "CD3D",
  ...
  ) + ylab("") + xlab("") + theme_cowplot(10) + theme(axis.text.x = element_text(
    angle = 90,
    hjust = 1,
    vjust = 0.5
  )) + scale_size(limits = c(0, 95), range = c(1, 6)) + scale_colour_gradient(low = "lightgray",
                                                                              high = "blue",
                                                                              limits = c(-2, 2.5))

sub <-
  subset(adrc_obj,
         subset = CellCluster == "CD4+ T cell" &
           celltype_cluster != "Proliferating CD4+ T cell ")
plot2 <-
  DotPlot(
    sub,
    group.by = "cellt_cl_tmp",
    feature = c(
      "MKI67",
      "CXCR5",
      "CD40LG",
  ...
  guides(
    ...
    hjust = 1,
    vjust = 0.5
  )) + scale_size(limits = c(0, 90), range = c(1, 6)) + scale_colour_gradient(low = "lightgray",
                                                                              high = "blue",
                                                                              limits = c(-2, 2.5))
...
plot4 <-
  DotPlot(
    ...
    feature = c(
      ...
      # Naive"IL4R",
      "CD79A",
      #neg in C-Mem  "CD27","CD19", "B2M", "CD74",
      "MARCKS",
      # M-memory# ND4
      "JCHAIN",
      "IGHA2",
...
plot5 <-
  DotPlot(
    subset(adrc_obj, subset = CellCluster %in% c("NK cell", "NKT-like cell")),
    group.by = "cellt_cl_tmp",
    feature = c("FCGR3A", # CD56-Dim
                "CD3D", # NKT-like cells
                "NCAM1" # CD56-Bright
                ),
                scale = T,
...
plot6 <-
  DotPlot(
    sub,
    group.by = "cellt_cl_tmp",
    feature = c("FCGR3A", # CD16 Monocytes
                "CD14",  # CD14 Monocytes
...
plot7 <-
  DotPlot(
    subset(adrc_obj, subset = CellCluster %in% c("Dendritic cell")),
    group.by = "cellt_cl_tmp",
    feature = c("CD1C", "FCER1A", # cDC2
                "ITM2C", "LILRA4",  # pDC
                "CADM1" # cDC1
                ),
                scale = T,
...
fig_marker <-
  plot_grid(
    ...
    plot_4,
    nrow = 4,
    rel_heights = c(1, .8, .7, 0.5)
  )

save_plot(
  "figures/supplement_3.svg",
  fig_marker,
  base_height = 200,
  base_width = 200,
  unit = "mm"
