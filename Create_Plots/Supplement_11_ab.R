...
CSF <-
  c(
    "Quanterix4plex.CSF___NFL",
    "Quanterix4plex.CSF___UCHL1",
    "Quanterix4plex.CSF___Tau",
    "Quanterix4plex.CSF___GFAP",
    "Quanterix3plex.CSF___Ab40",
    "Quanterix3plex.CSF___Ab42" ,
    "Quanterix3plex.CSF___Tau",
    "Quanterix.CSF___pTau181"
  )
col_data <- NA
for (meta_col in CSF) {
  meta_tmp <-
    meta_data[, colnames(meta_data) %in% c("SCMD", "Diagnosis", meta_col)]
  colnames(meta_tmp) <- c("SCMD", "Diagnosis", "meta_data")
  meta_tmp$meta <- meta_col
  if (is.na(col_data)[1]) {
    col_data <- meta_tmp
  } else {
    col_data <- rbind(col_data, meta_tmp)
  }
}
...
col_data$Disease_short <-
  factor(col_data$Disease_short,
...
plot <-
  ...
  geom_boxplot() +
  ggh4x::facet_grid2(. ~ str_replace(meta, ".*___", ""),
                     scales = "free",
                     independent = "y") +
...
plot <-
  ...
  geom_boxplot() +
  ggh4x::facet_grid2(
    . ~ str_replace_all(str_replace(meta, ".*___", ""), "\\.", " "),
    scales = "free",
    independent = "y"
  ) +
